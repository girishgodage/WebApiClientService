...
namespace GGSoftware.ApiClientService
{
    public class ServiceClient
    {
        ...
        /// <summary>
        /// Make a  POST or PUT  request (Form Data) to your web api  based on the ActionType and return a specific model 
        /// </summary>
        /// <typeparam name="T">Your data type that you want to send and get</typeparam>
        /// <param name="methodUrl">URL of your Web API that represetns the protected POST and PUT function </param>
        /// <param name="actionType"></param>
        /// <param name="keyValues"></param>
        /// <returns></returns>
        public async Task<T> SendFormAsync<T>(string methodUrl, ActionType actionType, params FormKeyValue[] keyValues)
        {
            HttpClient client = new HttpClient();
            ...
            {
                throw new ArgumentException("Key Values cannot be null or does not have values inside");
            }
            FormKeyValue[] arr_ = keyValues;
            for (int i = 0; i < arr_.Length; i++)
            {
                FormKeyValue formKeyValue = arr_[i];
                if (formKeyValue is FileFormKeyValue)
                {
                    FileFormKeyValue fileFormKeyValue = (FileFormKeyValue)formKeyValue;
                    multipartFormDataContent.Add(new StreamContent(fileFormKeyValue.FileStream, (int)fileFormKeyValue.FileStream.Length), formKeyValue.Key, fileFormKeyValue.FileName);
                }
                else
                {
                    StringFormKeyValue stringFormKeyValue = (StringFormKeyValue)formKeyValue;
                    multipartFormDataContent.Add(new StringContent(stringFormKeyValue.Value), stringFormKeyValue.Key);
                }
            }
            ...
            if (actionType == ActionType.POST)
                response = await client.PostAsync(methodUrl, multipartFormDataContent);
            else
                response = await client.PutAsync(methodUrl, multipartFormDataContent);
        ...
        /// <summary>
        /// Make a protected POST or PUT  request (Form Data) to your web api  based on the ActionType and return a specific model 
        /// </summary>
        /// <typeparam name="T">Your data type that you want to send and get</typeparam>
        /// <param name="methodUrl">URL of your Web API that represetns the protected POST and PUT function </param>
        /// <param name="actionType"></param>
        /// <param name="keyValues"></param>
        /// <returns></returns>
        public async Task<T> SendFormProtectedAsync<T>(string methodUrl, ActionType actionType, params FormKeyValue[] keyValues)
        {
            HttpClient client = new HttpClient();
